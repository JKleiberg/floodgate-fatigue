...
from src.configuration import N_HOURS, WIDTH, t, Gate
...
directory = '../data/06_transferfunctions/'+str(Gate.case)+\
            '/FRF_'+str(Gate.n_modes)+'modes'+str(Gate.case)+'.npy'
try:
...
def fatigue_gate(u_wind, h_sea, cat, ID=''):
    ...
    chunksize = 15
    cores = 4
    col_chunks = list(divide_chunks(Gate.stressneglist.tolist(), chunksize))[0][0][:Gate.n_modes]
...
def modalstresscontribution(modeshare, coords):
    """Histogram with ranges of mode stress contributions to all coordinates.
    ...
    Returns:
    fig: Figure with histogram of model contribution
...
def modalfatiguecontribution(wind, hsea, coords, cat=100, save=False):
    """Histogram with relative importance of all the modes to the total fatigue.

    Parameters:
    wind: Wind velocity of test event
    hsea: Average ater level velocity of test event
    coords: Coordinates of point of interest
    cat: Detail category of point of interest
    save: Whether to save the result
    
    Returns:
    fig: Figure with histogram of model contribution
    shares: List of modal contributions

    """
    coords = nearest(*coords)
    f, pqs_f, p_imp_t, _, _ = pressure(wind, hsea)
    res = stress_per_mode(f, pqs_f, p_imp_t, coords=coords)
    D_list = [fatigue(np.sum(res, axis=0)-mode, cat) for mode in res]
    total = fatigue(np.sum(res, axis=0), cat)
    ...
    plt.bar(modes, 100*shares, width=0.5, facecolor='white', edgecolor='black',
            label='Amount of fatigue damage lost by removing this mode')
    plt.ylim(0, 100)
    plt.xlabel('Mode')
    plt.ylabel('%')
