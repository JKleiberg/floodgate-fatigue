...
import numpy as np
import dill
from src.configuration import dx, dz, H_LAKE, n_x, n_z, n_freqs, f_max, cp, rho

def partition(lst, n):
    division = (len(lst)-1) / n
    return [0,*[round(division * (i+1)) for i in range(n)]]

def fluidmodes_free(GATE, Nf, depth):
    modes = np.arange(Nf)+1
    fshape_kx = np.zeros(Nf)
    fshape_x = np.zeros([Nf, int(GATE.WIDTH/dx)+1])
    for p in modes:
        fshape_kx[p-1] = (p-1)*np.pi/GATE.WIDTH
        fshape_x[p-1] = np.cos(fshape_kx[p-1]*GATE.x_coords)    
    fshape_kz = np.zeros(Nf)
    fshape_z = np.zeros([Nf, int(depth/dz)+1])
    zrange = tuple([GATE.z_coords<=depth])
    for r in modes:
        fshape_kz[r-1] = (2*r-1)*np.pi/(2*depth)
        fshape_z[r-1] = np.cos(fshape_kz[r-1]*GATE.z_coords[zrange]) 
    P = np.zeros([len(GATE.x_coords), len(GATE.z_coords[zrange]), Nf**2])
    DZ = np.zeros(Nf**2)
    for p in modes:
        for r in modes:
            pr = (p-1)*Nf+r
            P[:,:,pr-1] = np.outer(fshape_x[p-1],fshape_z[r-1].T)
            DZ[pr-1] = np.trapz(x=GATE.x_coords, y=fshape_x[p-1]**2)*np.trapz(x=GATE.z_coords[zrange], y=fshape_z[r-1]**2) 
    return fshape_kx, fshape_x.T, fshape_kz, fshape_z.T, P, DZ

def fluidmodes_closed(GATE, Nf, depth):
    modes = np.arange(Nf)+1
    fshape_kx = np.zeros(Nf)
    fshape_x = np.zeros([Nf, int(GATE.WIDTH/dx)+1])
    for p in modes:
...
def modalforce(GATE):
    intervals = np.zeros([n_x, n_z, int(1+GATE.WIDTH/dx), int(1+GATE.HEIGHT/dz)])
    for ii in range(n_x):
        for jj in range(n_z):
    ...
    for ii in range(n_x):
        for jj in range(n_z):
            for mode in range(GATE.n_modes):
                ...
                Fmode[mode, ii, jj] = np.trapz(x=GATE.x_coords[xrange],
                                             y=np.trapz(x=GATE.z_coords[zrange],
                                                        y=intervals[ii, jj, xrange, zrange]*-GATE.Wxz[xrange, zrange, mode],
                                                        axis=1))
    return Fmode
...
def fluid_structure_interaction(GATE, fluidmodes=[5,6,5]):
    GATE.ii_x = partition(GATE.x_coords, n_x)
    GATE.ii_z = partition(GATE.z_coords, n_z)

    f1_kx, f1_x, f1_kz, f1_z, f1_P, f1_DZ = fluidmodes_closed(GATE, fluidmodes[0], GATE.HEIGHT) # Closed because of overhang
    f2_kx, f2_x, f2_kz, f2_z, f2_P, f2_DZ = fluidmodes_free(GATE, fluidmodes[1],   GATE.HEIGHT)
    f3_kx, f3_x, f3_kz, f3_z, f3_P, f3_DZ = fluidmodes_free(GATE, fluidmodes[2],   H_LAKE)

    Fmode = modalforce(GATE)
    ...
    for mode in range(GATE.n_modes):
        for p1 in range(fluidmodes[0]):
            for p2 in range(fluidmodes[0]):
                ...
                Q[p, mode] = -np.trapz(x=GATE.x_coords,
                                     y=np.trapz(x=GATE.z_coords,
                                                y=np.multiply(GATE.Wxz[:,:,mode], f1_P[:,:,p]), axis=1)
                                    ) # Why minus sign?
    ...
    for mode in range(GATE.n_modes):
        for p1 in range(fluidmodes[2]):
            for p2 in range(fluidmodes[2]):
                ...
                T[p, mode] = -np.trapz(x=GATE.x_coords,
                                     y=np.trapz(x=GATE.z_coords[GATE.z_coords<=H_LAKE],
                                                y=np.multiply(GATE.Wxz[:, :f3_P.shape[1], mode], f3_P[:,:,p]), axis=1)
                                    ) # Why minus sign?
    ...
    for r in range(fluidmodes[1]):
        for p in range(fluidmodes[0]):
            R_intxL[r,p] = np.trapz(x=GATE.x_coords, y=f2_x[:,r]*f1_x[:,p])
            R_intzL[r,p] = np.trapz(x=GATE.z_coords, y=f2_z[:,r]*f1_z[:,p])
